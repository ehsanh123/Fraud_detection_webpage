...
#giveing a user by hand
# new_data = {
#     'USER_ID':['2416fa3c-0085-4d39-b2a8-e750506aa46f'],
#     'CREATED_DATE': ['2019-05-06 01:19:21.947'],
#     'OPENING_DATE':['2019-04-19 23:10:19.886'],
#     'since_last_tras':[27.013],
#     'min':[1555937267.676],
#     'avg_transaction_amount':[29.334],
#     'transaction_amount_std':[19.072693],
...
# # #pick a random user from dataset
# target_users = X_scaled[X_scaled['target'] == True]

# random_user = target_users.sample(n=1).copy()
# random_user1 = random_user
# col1=['C_unix_time','O_unix_time','B_unix_time']
# names1= ['CREATED_DATE','OPENING_DATE','BIRTH_DATE']
# for i in range(len(col1)):
#     scaled_value = np.array([random_user[col1[i]]]).reshape(-1, 1)
#     random_user[col1[i]]=scalers[col1[i]].inverse_transform([random_user[col1[i]]])
#     random_user[names1[i]] = pd.to_datetime(random_user[col1[i]], unit='s')

# random_user = random_user.drop(columns=col1)
# random_user = random_user.drop(columns=['F_unix_time','A_unix_time'])

# col1=['since_last_tras','min','avg_transaction_amount','transaction_amount_std',
#      'AMOUNT_GBP','TYPE','STATE','COUNTRY','CURRENCY']
# for i in range(len(col1)):
#     random_user[col1[i]]=scalers[col1[i]].inverse_transform([random_user[col1[i]]])

# col1=['TYPE','STATE','COUNTRY','CURRENCY']
# for i in range(len(col1)):
#     random_user[col1[i]]=encoders[col1[i]].inverse_transform(random_user[col1[i]].astype(int))

# # random_user = random_user.drop(columns=[ 'C_HOUR', 'C_MINUTE', 'A_DAY', 'A_HOUR', 'A_MINUTE',
# #        'B_YEAR', 'B_MONTH', 'B_DAY'])
# print(random_user1['target'])
# new_df=random_user
...
#         if printed: print(f'{col}: become {new_df[col].values}')

# #convert date to ...
# new_df['C_DATETIME'] = pd.to_datetime(new_df['CREATED_DATE'])
# new_df['C_HOUR'] = new_df['C_DATETIME'].dt.hour
# new_df['C_MINUTE'] = new_df['C_DATETIME'].dt.minute
# # if printed:print(f'CREATED_DATE: become {new_df[['C_HOUR']].values} H , {new_df[['C_MINUTE']].values} Min')

# new_df['O_DATETIME1'] = (pd.to_datetime(new_df['CREATED_DATE'])-pd.to_datetime(new_df['OPENING_DATE']))
# new_df['A_DAY'] = new_df['O_DATETIME1'].dt.days
# new_df['A_HOUR'] = new_df['O_DATETIME1'].dt.seconds // 3600 %24
# new_df['A_MINUTE'] = (new_df['O_DATETIME1'].dt.seconds //60 )%60
# # if printed:print(f'O_DATETIME1: become {new_df[['A_DAY']].values} Day ,
# # {new_df[['A_DAY']].values} H , {new_df[['A_MINUTE']].values} Min')

# new_df['B_DATETIME'] = pd.to_datetime(new_df['BIRTH_DATE'])
# new_df['B_YEAR'] = new_df['B_DATETIME'].dt.year
# new_df['B_MONTH'] = new_df['B_DATETIME'].dt.month
# new_df['B_DAY'] = new_df['B_DATETIME'].dt.day

# new_df['C_unix_time'] = (pd.to_datetime(new_df['CREATED_DATE'])).astype('int64') / 10**9  # Convert to seconds
# new_df['O_unix_time'] = (pd.to_datetime(new_df['OPENING_DATE'])).astype('int64') / 10**9  # Convert to seconds
# # print(f'C_unix_time: become {new_df[['C_unix_time']].values} , O_unix_time become  {new_df[['O_unix_time']].values}')

# new_df['A_unix_time'] = new_df['C_unix_time'] - new_df['O_unix_time']
# new_df['B_unix_time'] = (pd.to_datetime(new_df['BIRTH_DATE'])).astype('int64') / 10**9  # Convert to seconds
# # print(f'A_unix_time: become {new_df[['A_unix_time']].values} , B_unix_time become  {new_df[['B_unix_time']].values}')

# #  df['since_last_tras'] = df['since_last_tras'].dt.total_seconds()
# new_df['F_unix_time'] = new_df['C_unix_time'] - new_df['min']
# # print(f'since_last_tras: become {new_df[['since_last_tras']].values} , F_unix_time become  {new_df[['F_unix_time']].values}')


# new_df['Dif1'] = np.where(new_df['AMOUNT_GBP'] > new_df['avg_transaction_amount'],
#         new_df['AMOUNT_GBP'] - new_df['avg_transaction_amount'],
#         new_df['avg_transaction_amount'] - new_df['AMOUNT_GBP'])

# new_df['Dif2'] = np.where(new_df['AMOUNT_GBP'] > new_df['avg_transaction_amount'],
#                       new_df['Dif1'] - 2 * new_df['transaction_amount_std'],
#                       new_df['Dif1'] - 2 * new_df['transaction_amount_std'])
...
#          'O_DATETIME1',
#          'B_DATETIME']
# X3 = new_df.drop(columns=coli2)
# X2 = X_scaled.drop(columns=['target'])
# X3=X3[X2.columns.values]
# print(X3.iloc[0])
# # data2 = get_data_ready_for_model(random_user1)
# # X_2 = X_scaled.drop(columns=['target'])
# # data_model=data2[X_2.columns.values]
# # print(data2.iloc[0])
# y_user = model.predict(X3.drop(columns=['USER_ID']))
# print(y_user)
